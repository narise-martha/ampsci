...
// < lb, mb | Y_{L,M} | la, ma >
double ME_spherical_harm(int twoL, int twoM, int lb, int la, int twomb, int twoma) {
    ...
    return sqrt((twoL+1)/4*M_PI)*pow(-1,lb-mb)*Angular::threej_2(2*lb,twoL,2*la,-twomb,twoM,twoma)*reduced_ME;
}
...
// sigma_x
double ang_int_sig_x(int twoL, int twoJ, int twoM, int lambda, int lb, int twojb, int twomb, int la, int twoja, int twoma){
...
// sigma_y
std::complex<double> ang_int_sig_y(int twoL, int twoJ, int twoM, int lambda, int lb, int twojb, int twomb, int la, int twoja, int twoma){

  double term_1 = -cg(2*lb,1,twojb,twomb-1,1,twomb)*cg(2*la,1,twoja,twoma+1,-1,twoma)*ME_spherical_harm(twoL,twoM-2*lambda,lb,la,twomb-1,twoma+1);
  double term_2 = cg(2*lb,1,twojb,twomb+1,-1,twomb)*cg(2*la,1,twoja,twoma-1,1,twoma)*ME_spherical_harm(twoL,twoM-2*lambda,lb,la,twomb+1,ma-1);
  
  return i*cg(twoL,2,twoJ,twoM-(2*lambda),2*lambda,twoM)*(term_1 + term_2);
}

// sigma_z
double ang_int_sig_z(int twoL, int twoJ, int twoM, int lambda, int lb, int twojb, int twomb, int la, int twoja, int twoma){
...
std::complex<double> ME_Y_JLM(const DiracSpinor &Fa, const DiracSpinor &Fb, const auto jL, int twoL, int twoJ, int twoM, int twomb, int twoma){
...
double abs_reduced_ME_alpha_a(const DiracSpinor &Fa, const DiracSpinor &Fb, int sigma, int twoJ){
  
  std::complex<double> full_ME;

  if (sigma==-1){
    full_ME = sqrt(twoJ/(2*twoJ+2))*ME_Y_JLM(Fa,Fb,jL,twoJ-2,twoJ,0,1,1) - sqrt((twoJ+1)/(2*twoJ+2))*ME_Y_JLM(Fa,Fb,jL,twoJ+2,twoJ,0,1,1);
  }

  elseif (sigma==0){
    full_ME = ME_Y_JLM(Fa,Fb,jL,twoJ,twoJ,0,1,1);
  }

  elseif (sigma==-1){
    full_ME = sqrt((twoJ+1)/(2*twoJ+2))*ME_Y_JLM(Fa,Fb,jL,twoJ-2,twoJ,0,1,1) + sqrt(twoJ/(2*twoJ+2))*ME_Y_JLM(Fa,Fb,jL,twoJ+2,twoJ,0,1,1);
  }
  ...
  return pow(abs(pow(-1,0.5*(double(Fa.twoj() - Fb.twoj())))*(1/Angular::threej_2(twojb,twoJ,twoja,-1,0,1))*full_ME),2);
}
